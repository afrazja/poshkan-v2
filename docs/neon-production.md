...
No source freeze, domain promotion, or Supabase pause has been performed. Automatic approval review requires the owner to explicitly approve the temporary production write block before cutover. The private migration helper contains a verified-data guard and an `unfreeze` mode; it preserves all source records and only installs reversible write-rejection triggers. Do not switch while source writes can continue. The local production preview was stopped before the final check.
