...
export async function serviceWork<T>(work: Parameters<typeof transaction<T>>[1], cache = false) {
  ...
  if (!servicesEnabled() || !userId) throw new Error('Neon services are disabled');
  return transaction(userId, async c => {
    if (!cache) await c.query(`SELECT ${databaseSchema()}.actor()`);
    return work(c);
...
export function createNeonServiceClient(scope: 'services' | 'cache' = 'services'): SupabaseClient {
    ...
    global: { fetch: async (input, init) => {
      try {
        ...
        if (url.pathname.includes('/rpc/')) {
          ...
          if(name==='fx_close') {
            const position=await serviceWork(async c=>(await c.query(`SELECT id,account_id,symbol FROM ${databaseSchema()}.fx_positions WHERE id=$1 AND status=$2`,[args.p_position_id,'open'])).rows[0]);
            if(!position)throw new Error('Position not found');
            const q=await yahoo.quote(String(position.symbol));
            const price=checkedQuote(position.symbol,q,true);
            await serviceWork(c=>c.query(`SELECT ${databaseSchema()}.service_check($1,$2,$3,$4,$5,$6)`,['POSITION',position.id,position.account_id,position.symbol,price,q.regularMarketTime]));
            const receipt=await serviceWork(async c=>(await c.query(`SELECT status,pnl FROM ${databaseSchema()}.fx_positions WHERE id=$1`,[position.id])).rows[0]);
          ...
          const result=await serviceWork(async c=>(await c.query(`SELECT ${databaseSchema()}.${name}(${keys.map((_,i)=>'$'+(i+1)).join(',')}) AS result`,keys.map(key=>args[key]??null))).rows[0].result);
  ...
  const users = async () => serviceWork(async c => {
    const { rows } = await c.query(`SELECT * FROM ${databaseSchema()}.service_user()`);
...
export async function captureDelivery(channel: 'email' | 'push', recipient: string, payload: object) {
  await serviceWork(async c => {
    await c.query(`SELECT ${databaseSchema()}.capture_delivery($1,$2,$3::jsonb)`, [channel, recipient, JSON.stringify(payload)]);
  });
}
