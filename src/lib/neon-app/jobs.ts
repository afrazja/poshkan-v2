...
export async function runNeonMarketCheck() {
  const candidates: {kind:string;id:string;accountId:string;symbol:string}[] = await serviceWork(async c => (await c.query('SELECT poshkan_trade_test.order_candidates() AS items')).rows[0].items);
  ...
  for (const item of candidates) {
    ...
    const result = await serviceWork(async c => (await c.query('SELECT poshkan_trade_test.service_check($1,$2,$3,$4,$5,$6) AS result', [item.kind,item.id,item.accountId,item.symbol,price,at])).rows[0].result);
  ...
  for (const alert of alerts || []) {
    ...
    if (user?.email) await sendEmail(user.email,`${alert.symbol} price alert`,alertEmailHtml({symbol:alert.symbol,condition:alert.condition,targetPrice:target,triggeredPrice:price,appUrl:'http://localhost:3025'}));
  }
  return {...summary,triggered,delivery:'captured locally'};
}
