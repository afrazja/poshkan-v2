...
export async function readOrders(): Promise<OrderState | null> {
  ...
  return transaction(userId, async c => {
    ...
    const ready = await c.query(`SELECT to_regprocedure('${databaseSchema()}.order_state()') IS NOT NULL AS ready`);
    return ready.rows[0].ready ? (await c.query(`SELECT ${databaseSchema()}.order_state() AS state`)).rows[0].state : null;
  });
}
...
export async function saveOrder(requestId: unknown, input: unknown): Promise<Record<string,string>> {
  ...
  return transaction(userId, async c => (await c.query(`SELECT ${databaseSchema()}.order_command($1,$2::jsonb) AS result`,[request,command])).rows[0].result);
}
...
export async function checkOrders() {
  const userId = await actor();
  const candidates: Candidate[] = await transaction(userId, async c => (await c.query(`SELECT ${databaseSchema()}.order_candidates() AS items`)).rows[0].items);
  ...
  for (const item of candidates) {
    ...
    try {
      ...
      const result: CheckResult = await transaction(userId, async c => (await c.query(
        `SELECT ${databaseSchema()}.check_order($1,$2,$3,$4,$5::numeric,$6::timestamptz) AS result`,
