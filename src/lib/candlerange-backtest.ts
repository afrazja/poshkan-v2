...
export interface CandleRangeTrade {
  symbol: string;
  ...
  exitTime: string;
  exit: number;
  r: number; // +rr on a win (target is the opposite edge, so R varies), -1 on a loss
  win: boolean;
}

export interface CandleRangeBtSymbol {
  symbol: string;
  trades: CandleRangeTrade[];
  n: number;
  wins: number;
  winRate: number;
  totalR: number;
...
export interface CandleRangeBtResult {
  perSymbol: CandleRangeBtSymbol[];
  n: number;
  wins: number;
  winRate: number;
  totalR: number;
...
async function backtestSymbol(symbol: string, params: CandleRangeParams): Promise<CandleRangeBtSymbol> {
  ...
  let i = LOOKBACK;
  while (i < c.length) {
    const ev = evaluateCandleRangeAt(symbol, c.slice(Math.max(0, i - LOOKBACK), i + 1), params);
    if (
      ev.status === "signal" &&
      ev.direction &&
      ev.entry != null &&
...
export async function backtestCandleRange(
  symbols: string[],
  params: CandleRangeParams = CANDLERANGE_DEFAULTS
): Promise<CandleRangeBtResult> {
