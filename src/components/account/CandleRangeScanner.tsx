...
import {
  getCandleRangeData,
  saveCandleRangeSettings,
  backtestCandleRangeAction,
  refreshCandleRangeRead,
  type CandleRangeSettings,
  type CandleRangeSignal,
  type CandleRangeStatusItem,
} from "@/app/dashboard/[accountId]/candlerange-actions";
import type { CandleRangeBtResult } from "@/lib/candlerange-backtest";
...
export default function CandleRangeScanner({
  accountId,
  accountType,
  initialSettings,
  initialSignals,
  ...
  const [settings, setSettings] = useState<CandleRangeSettings | null>(initialSettings);
  const [signals, setSignals] = useState<CandleRangeSignal[]>(initialSignals);
  const [saving, startSave] = useTransition();
  const [saved, setSaved] = useState(false);

  const [bt, setBt] = useState<CandleRangeBtResult | null>(null);
  ...
  const [confirmCandle, setConfirmCandle] = useState(initialSettings?.confirm_candle ?? true);
  ...
  useEffect(() => {
    const id = setInterval(async () => {
      ...
      const data = await getCandleRangeData(accountId);
      if (data) {
  ...
  async function runBacktest() {
    ...
    setBt(null);
    try {
      const res = await backtestCandleRangeAction({
        accountId,
        symbols,
  ...
  async function runScanNow() {
    setScanning(true);
    try {
      const res = await refreshCandleRangeRead(accountId);
      if (res.error) {
  ...
  const save = () =>
    startSave(async () => {
      const res = await saveCandleRangeSettings({
        accountId,
        enabled,
        mode,
        symbols,
  ...
  return (
    <ScannerCard
      icon={<ScannerIcon kind="candlerange" size={18} />}
      name="Candle Range"
    ...
    >
      ...
      <ScannerInfo
        ...
        reading="'signal' = price is at an edge with a confirming candle. Otherwise it's mid-range, not yet a clean range, or has broken out."
        judge="Range trading wins often but gives back fast on breakouts — weigh win rate WITH profit factor and max drawdown."
      />
      <button
        onClick={runScanNow}
        disabled={scanning}
      ...
      <div className="mt-3 space-y-3 rounded-lg border border-border bg-background p-3">
        ...
        <SettingsSection title="Entry rules">
          ...
          <label className="col-span-2 flex items-start gap-2 text-sm">
            <input
              type="checkbox"
              checked={confirmCandle}
              onChange={(e) => setConfirmCandle(e.target.checked)}
              className="mt-0.5"
            />
            <span>
              Require a confirmation candle
              <span className="block text-[11px] text-muted">
                Only enter when the candle at the edge closes in the bounce direction (bullish at support,
                bearish at resistance). Fewer, higher-quality entries.
              </span>
            </span>
          </label>
        </SettingsSection>
      ...
      {/* Signal history */}
      <SignalHistory signals={signals} interval="15min" />
    </ScannerCard>
  );
}
