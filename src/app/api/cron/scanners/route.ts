...
export async function GET(request: Request) {
  const [smcRes, oteRes, trendRes, meanrevRes, aiRes] = await Promise.all([
    smcScan(request),
    oteScan(request),
    trendScan(request),
    meanrevScan(request),
  ...
  const [smc, ote, trend, meanrev, ai] = await Promise.all([
  ...
  return NextResponse.json({ smc, ote, trend, meanrev, ai });
}
