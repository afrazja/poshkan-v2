...
import {
  evaluateCandleRangeSymbol,
  CANDLERANGE_DEFAULTS,
  type CandleRangeEval,
  type CandleRangeParams,
...
// Deterministic Candle-Range (box) scanner (no AI). Bundled into /api/cron/scanners.
export async function GET(request: Request) {
  ...
  let settings: CandleRangeRow[];
  try {
    const { data, error } = await db.from("candlerange_settings").select("*").eq("enabled", true);
    if (error) return NextResponse.json({ skipped: "candlerange_settings unavailable", detail: error.message });
    settings = (data ?? []) as CandleRangeRow[];
  } catch {
    return NextResponse.json({ skipped: "candlerange_settings table missing" });
  }
  ...
  const evalCache = new Map<string, CandleRangeEval>();
  const evalFor = async (symbol: string, p: CandleRangeParams): Promise<CandleRangeEval> => {
    const ckey = `${symbol}|${p.rangePeriod}|${p.edgeZone}|${p.slAtrMult}|${p.confirmCandle ? 1 : 0}`;
    const hit = evalCache.get(ckey);
    if (hit) return hit;
    const res = await evaluateCandleRangeSymbol(symbol, p);
  ...
  for (const s of settings) {
    ...
    const params: CandleRangeParams = {
      ...CANDLERANGE_DEFAULTS,
      rangePeriod: Math.min(100, Math.max(8, Math.round(Number(s.range_period) || CANDLERANGE_DEFAULTS.rangePeriod))),
      edgeZone: Math.min(0.45, Math.max(0.1, Number(s.edge_zone) || CANDLERANGE_DEFAULTS.edgeZone)),
      slAtrMult: Math.min(3, Math.max(0.1, Number(s.sl_atr_mult) || CANDLERANGE_DEFAULTS.slAtrMult)),
      confirmCandle: s.confirm_candle ?? CANDLERANGE_DEFAULTS.confirmCandle,
    };
    ...
    if (acc.type !== "crypto") {
      ...
      if (!probe?.isMarketOpen) {
        await db.from("candlerange_settings").update({ last_run_at: new Date().toISOString() }).eq("account_id", s.account_id);
        continue;
      }
    }

    const evals: CandleRangeEval[] = [];
    ...
    for (const ev of evals) {
        ...
        if (open.length >= (Number(s.max_open) || 2)) {
          ...
          if ((tradesToday ?? 0) >= (Number(s.max_per_day) || 5)) {
            ...
            if (units <= 0) {
              ...
              if (!error) {
                if (newId) await db.from("fx_positions").update({ source: "candlerange" }).eq("id", newId);
                ...
                await markExecuted(db, "candlerange_signals", claimId, { entry: liveRate, stop: sl, take_profit: tp });
                placed++;
                didTrade = true;
                try {
                  await sendPushToUser(acc.user_id, {
                    title: `📦 Range auto-trade: ${ev.direction} ${symbolLabel(symbol)}`,
      ...
      try {
        await sendPushToUser(acc.user_id, {
          title: autoMode
            ? `📦 Range: ${ev.direction} ${symbolLabel(symbol)} — auto-trade skipped`
            : `📦 Range signal: ${ev.direction} ${symbolLabel(symbol)} (${ev.rr}R)`,
          body: autoMode
            ? `Auto-trade skipped: ${why}. Setup: entry ~${fmt(ev.entry)} · SL ${fmt(ev.stop)} · TP ${fmt(ev.takeProfit)}.`
            : `Entry ~${fmt(ev.entry)} · SL ${fmt(ev.stop)} · TP ${fmt(ev.takeProfit)}. ${ev.reason}`,
          url: `/dashboard/${acc.id}`,
        });
      } catch {}
    }
